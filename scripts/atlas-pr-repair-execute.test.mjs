...
test("repository repair policy is disabled after the pilot", () => {
  assert.throws(() => validateExecutionPolicy(policySource), /disabled/);
  assert.equal(policy.enabled, false);
  assert.equal(policy.pilot_enabled, false);
  assert.equal(policy.auto_merge, false);
  assert.deepEqual(policy.pilot_allowed_pr_numbers, []);
...
function enabledPolicySource() {
  return policySource
    ...
    .replace("  pilot_allowed_triggering_actors:", "  pilot_allowed_triggering_actors:\n    - \"operator\"")
