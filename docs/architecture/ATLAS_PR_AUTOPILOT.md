...
Der Plan-Workflow checkt ausschließlich `main` mit `persist-credentials: false` aus und lädt PR-, Datei-, Check- und Review-Fakten über die GitHub API. Er verifiziert die Supervisor-Beobachtung gegen den auslösenden Lauf, berechnet die Supervisor-Entscheidung aus vertrauenswürdigem Code erneut und verlangt denselben Status/Fingerprint. Der aktuelle PR-Head muss dem vom Supervisor beobachteten vollständigen SHA entsprechen. Ein Head-Wechsel unterdrückt die automatische Planung; ein bereits erzeugter Plan für einen alten SHA ist nur noch informativ und niemals eine Ausführungsfreigabe.
